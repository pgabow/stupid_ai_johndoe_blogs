        ...
        <div className='px-4 lg:px-0'>
          <div className='flex items-center mb-8 w-full'>
            <div className='flex items-center w-full lg:w-auto mr-8'>
              ...
              />
              <p className='inline align-middle ml-2 subtitle'>{post.author.name}</p>
            </div>
            <div className='font-medium flex items-center'>
              <FaCalendarAlt className='w-5 h-5 mr-2 text-blue-400' />
              <span className='subtitle'>{moment(post.createdAt).format('DD/MM/YYYY')}</span>
            </div>
          </div>
          <div className='mb-4'>
            {post.categories.map((category) => (
              <Link href={`/category/${category.slug}`} key={category.slug}>
                <span className='inline-block mr-3 mb-3 category'>#{category.name}</span>
              </Link>
            ))}
          </div>
          <h2>{post.title}</h2>
          <RichText
            ...
            renderers={{
              ...
              // quote block
              blockquote: ({ children }) => (
                <blockquote className='border-l-4 border-sky-700 pl-4 mb-4'>{children}</blockquote>
              ),
              // code
              code: ({ children }) => (
              ...
              // code block
              code_block: ({ children }) => (
                <pre className='mb-4 bg p-2 rounded overflow-hidden'>{children}</pre>
              ),
              // unordered list
