    ...
    return (
        <div className='bg supports-backdrop-blur:bg-white/95 dark:bg-zinc-900/75 border border-sky-900 rounded-lg p-0 lg:p-8 pb-12 mb-8'>
            ...
            <Link href={ `/post/${post.slug}` } title={ post.title }>
                <h3 className='transition duration-400 text-center px-2 mb-3 mt-2 cursor-pointer hover:text-blue-400 font-semibold'>
                    { post.title }
                </h3>
            </Link>
            ...
            {/* <div className='text-center'>
                <Button href={ `/post/${post.slug}` }>
                    Read <FaArrowRight className="w-4 h-4 inline-block group-hover:translate-x-1 transition-all" />
                </Button>
            </div> */}
        </div>
    )
